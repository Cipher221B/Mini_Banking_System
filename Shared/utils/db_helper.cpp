...
void DB_Helper::bind_parameter_double(SQLHSTMT hstmt, int col, double& value)
{
    SQLBindParameter(hstmt, col, SQL_PARAM_INPUT, SQL_C_DOUBLE, SQL_DECIMAL, 19, 4, &value, 0, NULL);
}
...
{
    SQLBindParameter(hstmt, col, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, value.size(), 0, (SQLPOINTER)value.c_str(), 0, &value_len);
}
...
void DB_Helper::bind_col_int(SQLHSTMT hstmt, int col, SQLLEN& value_ind, int& value)
{
    SQLBindCol(hstmt, col, SQL_C_LONG, &value, 0, &value_ind); //declare location get data each column
}
...
void DB_Helper::get_data_int(SQLHSTMT hstmt, int col, int& value)
{
